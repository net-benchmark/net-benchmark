...
**What’s the difference between `benchmark` and `load-test`?**
`benchmark` runs a fixed number of iterations per target to get detailed per‑request timing and security headers. `load-test` runs for a set duration with configurable concurrency or RPS to measure throughput, saturation, and tail latency under sustained load. Use `benchmark` for one‑off performance audits; use `load-test` for capacity planning and stress testing.
