...
| 模式         | 功能                                      | 使用场景                          |
|--------------|-------------------------------------------|-----------------------------------|
| `throughput` | 尽可能压测到 `--max-concurrency` 上限     | 寻找天花板                        |
| `sustained`  | 在 `--duration` 内保持固定的 `--rps`      | SLA / 容量验证                    |
| `ramp-up`    | 逐步增加并发，然后保持峰值                | 渐进式寻找崩溃点                  |
...
net-benchmark http load-test \
  ...
  --formats csv,excel,pdf
```
`--max-total-rps` 是一个*安全上限*，不是目标速率——如果要固定速率请使用 `sustained`。它的存在是因为对于非常快的目标（localhost、网格 sidecar）没有其他东西能限制请求速率。默认值为 `ramp-concurrency * 50`，通常足够大，不会对实际的网络绑定服务触发。

**对比多个目标（例如 canary vs stable）**
```bash
net-benchmark http load-test \
...
```
每个目标在其自己的连接池中并发运行。Excel 导出会生成一个对比表以及每个目标的原始请求和时间线表。
...
**`benchmark` 和 `load-test` 有什么区别？**  
`benchmark` 对每个目标运行固定次数的迭代，以获取详细的每请求计时和安全头信息。`load-test` 在设定的持续时间内运行，具有可配置的并发度或 RPS，以测量持续负载下的吞吐量、饱和度和尾延迟。使用 `benchmark` 进行一次性性能审计；使用 `load-test` 进行容量规划和压力测试。
